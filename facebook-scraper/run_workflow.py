...
async def step1_scrape():
    ...
    await send_telegram(f"🔄 ขั้นตอน 1 - กำลัง scrape โพสจาก 6 กลุ่ม...")
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8-sig') as f:
        w = csv.writer(f)
        w.writerow(['No', 'Group', 'Post URL', 'Message', 'PostTime', 'ScrapedAt'])
    ...
    await send_telegram(f"✅ ขั้นตอน 1 เสร็จ: เก็บได้ {len(posts)} โพส\n\n🔄 กำลังดำเนินขั้นตอน 2...")
    
    return posts

async def step2_filter(posts):
    """Step 2: Filter posts with MiniMax AI"""
    print(f"[{thai_time_str()}] 🔍 ขั้นตอน 2 - กรองด้วย MiniMax AI...")
    ...
    for i, p in enumerate(posts, 1):
        msg = p.get('message', '')
        result = minimax_analyze(msg, p.get('url',''), p.get('group',''))
        
        p['score'] = result.get('score', 0)
        p['is_lead'] = result.get('is_lead', False)
        p['reason'] = result.get('reason', '')[:80]
        
        if result.get('is_lead'):
            leads.append(p)
            emoji = "🟢"
            print(f"{emoji} {i}/{len(posts)}: score={p['score']} | {msg[:50]}...")
        else:
            not_leads.append(p)
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8-sig') as f:
        w = csv.writer(f)
        w.writerow(['No', 'Status', 'Group', 'Post URL', 'Message', 'Score', 'Reason'])
        for i, p in enumerate(all_posts, 1):
            status = '✅ Lead' if p.get('is_lead') else '❌ Rejected'
            w.writerow([i, status, p.get('group',''), p.get('url',''), p.get('message','')[:200], p.get('score',0), p.get('reason','')])
