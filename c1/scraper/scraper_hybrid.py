...
# Import Ultra Accurate AI Filter (Keyword-based)
from ai_buyer_filter import ultra_analyze, extract_contact

# Alias for compatibility
minimax_analyze = ultra_analyze

# ==================== CONFIG ====================
THAI_OFFSET = timedelta(hours=7)
MAX_SCROLLS = random.randint(5, 9)
MAX_RETRIES = 2
BROWSER_REUSE_LIMIT = 2
WORKER_COUNT = 3
DELAY_MIN = 2
DELAY_MAX = 5
LEADS_DIR = "/root/.openclaw/workspace/c1/scraper/leads"
PENDING_DIR = "/root/.openclaw/workspace/c1/scraper/pending"
...
found_lock = Lock()
stats_lock = Lock()
stats = {'total_leads': 0, 'total_scraped': 0, 'groups_completed': 0, 'start_time': time.time()}
cookies = None
shutdown_flag = False
...
def launch_browser(playwright):
    ...
    # Don't set proxy here - set it in context instead
...
def new_context(browser, cookies, proxy_config=None):
    proxy_args = {}
    if proxy_config:
    ...
    while not shutdown_flag:
        ...
        try:
            with sync_playwright() as p:
                ...
                ctx = new_context(browser, cookies, proxy_config)
                page = ctx.new_page()
...
def main():
    ...
    # Send admin alert with detailed leads info
    try:
        from admin_alerts import send_fbuyer_result_alert
        send_fbuyer_result_alert(len(all_leads), leads_count, all_leads)
    except Exception as e:
